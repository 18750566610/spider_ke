import fanbox.MANA.file as ALLPage
import requests
...
def getWorkLink(link, writer, company):
    # link = transFrom()
    print("worklink:", link)
    ...
    for i in range(len(link)):
        ...
        for j in bs.find_all(lambda tag: tag.name == 'div' and tag.get('class') == ['post__content']):
            ...
            # count += 1
            # print(txt)
            path = f"F:/色色区/画集全/{writer}_{company}/txt/"
            if not os.path.exists(path):
                os.makedirs(path)
            # if count == 0:
            #     txt111 = open(f"{path}{writer}_{count}_content.txt", "w", encoding="utf-8")
            #     txt111.truncate(0)
            #     # count1 = i
            #     text1 = f"{i}个info"
            #     txt111.write(text1)
            ...
            if count1 == 0:
                txt111 = open(f"{path}{writer}_{count}_content.txt", "w+", encoding="utf-8")
                txt111.truncate(0)
            txt = open(f"{path}{writer}_{count}_content.txt", "a+", encoding="utf-8")
        ...
        count = 0
        txttxt = []
        path1 = f"D:/ZANZHU/{writer}_{company}/link/"
        if not os.path.exists(path1):
            os.makedirs(path1)
        txt1111 = open(f"{path1}{writer}_content.txt", "w+", encoding="utf-8")
        for j1 in bs.findAll("a"):
            # for j in bs.find_all(lambda tag: tag.name == 'div'):
            # print("j", j)
            txt36 = j1["href"]
            # txt136 = str(txt36)
            # print("txt36", txt36) # 输出链接文本内容
            txttxt.append(txt36)
            ...
            int(count1)
            print(count1)
            text10 = f"{count1}个info"
            # txt.write(str(count1))
            txt1111.write(text10)

            txt1111.write("\n")
            txt1111.write("\n")
            count += 1
        json131.append(txttxt)
        json.dump(json131, txt1111, indent=5)
...
#     # downloadWorkLink()
#     # downloadImgLink()
#     link = ['https://kemono.party/fanbox/user/11936535/post/4849320',
#             'https://kemono.party/fanbox/user/11936535/post/4671114',
#             'https://kemono.party/fanbox/user/11936535/post/4651097']
#     getWorkLink(link, "thanabis", "fanbox")
